...
func WebRPCSchemaHash() string {
	return "9e4b584f053fac80af73bcb9d84d5f9636e9932d"
}
...
type QSO struct {
	...
	Station_callsign string `json:"station_callsign"`
	Dx_callsign      string `json:"dx_callsign"`
	Freq_hz          uint64 `json:"freq_hz"`
	Time             string `json:"time"`
	Mode             string `json:"mode"`
	Rst_sent         string `json:"rst_sent"`
	Rst_rcvd         string `json:"rst_rcvd"`
	Exch_sent        string `json:"exch_sent"`
	Exch_rcvd        string `json:"exch_rcvd"`
}

type MCLService interface {
	GetQSOList(ctx context.Context) ([]*QSO, error)
	UpdateQSO(ctx context.Context, qso *QSO) (bool, error)
}
...
type mCLServiceServer struct {
	MCLService
}

func NewMCLServiceServer(svc MCLService) WebRPCServer {
	return &mCLServiceServer{
		MCLService: svc,
	}
}

func (s *mCLServiceServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	ctx = context.WithValue(ctx, HTTPResponseWriterCtxKey, w)
	ctx = context.WithValue(ctx, HTTPRequestCtxKey, r)
	ctx = context.WithValue(ctx, ServiceNameCtxKey, "MCLService")
	...
	switch r.URL.Path {
	case "/rpc/MCLService/GetQSOList":
		s.serveGetQSOList(ctx, w, r)
		return
	case "/rpc/MCLService/UpdateQSO":
		s.serveUpdateQSO(ctx, w, r)
		return
	default:
...
func (s *mCLServiceServer) serveGetQSOList(ctx context.Context, w http.ResponseWriter, r *http.Request) {
...
func (s *mCLServiceServer) serveGetQSOListJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	var err error
	ctx = context.WithValue(ctx, MethodNameCtxKey, "GetQSOList")
	...
	func() {
		...
		ret0, err = s.MCLService.GetQSOList(ctx)
	}()
	respContent := struct {
		Ret0 []*QSO `json:"qsos"`
	}{ret0}
...
func (s *mCLServiceServer) serveUpdateQSO(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	header := r.Header.Get("Content-Type")
	i := strings.Index(header, ";")
	if i == -1 {
		i = len(header)
...
func (s *mCLServiceServer) serveUpdateQSOJSON(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	var err error
	ctx = context.WithValue(ctx, MethodNameCtxKey, "UpdateQSO")
	reqContent := struct {
		Arg0 *QSO `json:"qso"`
	}{}
	...
	func() {
		...
		ret0, err = s.MCLService.UpdateQSO(ctx, reqContent.Arg0)
	}()
	respContent := struct {
