...
int set_ant(RIG *myrig, int vfo, int ant)
{
	value_t dummy;
	dummy.s = NULL;
	int res = rig_set_ant(myrig, vfo, ant, dummy /* option */);
	return res;
}
...
{
	// This won't work, just to make it compilable with hamlib 4.x
	int res = rig_get_ant(myrig, vfo, ant, NULL, NULL, NULL, NULL);
	return res;
}
