...
* For iOS and MacOS: Xcode, via CocoaPods.
  * See the documentation in ios/gui_lib.podspec.
  * See the documentation in macos/gui_lib.podspec.
...
(`src/gui_lib.h`) by `package:ffigen`.
Regenerate the bindings by running `flutter pub run ffigen --config ffigen.yaml`.

## Invoking native code

Very short-running native functions can be directly invoked from any isolate.
For example, see `sum` in `lib/gui_lib.dart`.

Longer-running functions should be invoked on a helper isolate to avoid
dropping frames in Flutter applications.
For example, see `sumAsync` in `lib/gui_lib.dart`.
